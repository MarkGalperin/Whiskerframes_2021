function [ANG,PTS,WID,LAB] = pp1_janelia(MSR,fillnans,omitlast,extrasel,numfile)
...
%         coordinates for N whiskers over T frames
    ...
    for t = 0:T
        ...
        Awid = [double(labsel) ; widsel];
        Alab = [double(labsel) ; labsel];
        ...
        if isempty(labsel)
            ...
            %assign and log wid
            init_row = NaN(1,N);
            init_row(A_swid(1,:)+1) = A_swid(2,:); %+1 to turn whisker id into MATLAB indices
            WID(t+1,:) = init_row;
            
            %assign and log label
            init_row = NaN(1,N);
            init_row(A_slab(1,:)+1) = A_slab(2,:); %+1 to turn whisker id into MATLAB indices
            LAB(t+1,:) = init_row;
        end 
    end
    ...
    if extrasel
        ...
        %reconstruct array
        B = [fidsel',lensel',anglesel',xsel',ysel',widsel',labsel'];
        ...
        for t = 0:T
            ...
            elseif size(Bsel,1) == 1
                ...
                wid_log(t+1,1) = Bsel(1,6);
                lab_log(t+1,1) = Bsel(1,7);
            ...
            %deal with multiple selection
                ...
                wid_log(t+1,1) = Bsel(ind,6);
                lab_log(t+1,1) = Bsel(ind,7);
            end
