...
X = 1:500; %1:size(DATA_i.angles,1);
...
%dynamic constraints
C.R = 0.2; %xy jump tolerance (NEW: this now only makes sense to be less than the search box, otherwise the searchbox defines the constraint)
C.accel = 0.1; %xy acceleration tolerance
C.dtheta = (pi/2); %theta jump tolerance
C.ddtheta = 0.1; %theta acceleration constraint
%search parameters
C.res = [0.02, 0.02, 0.002]; %search resolution for r1,r2,th (or m1,m2,th)
...
C.axis = 'm'; %center axis for control frame. Can be 'm' or 'r'
