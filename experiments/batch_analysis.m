...
RUN = [run_writeresults,...
       run_makefigures,...
       run_correlations]; %add as needed

%% LOAD DATA
% choose batch directory
batch_dir = '../output/trial_data/BATCH_Oct10_filt/'; %make sure to end with "/"
...
for set_i = 1:length(SETS)
    ...
    for trial_i = 1:length(set)
        ...
        newrow = struct('Trial',trial_i,...
                        ...
                        'overc_cnt',sum(trial.TRIAL(Nopt).overc),...
        ...
        if RUN(2)
            ...
            file = '500_p1inv_bio'; %figure batch directory name
            for opt = 1:Nopt
                %file setup
